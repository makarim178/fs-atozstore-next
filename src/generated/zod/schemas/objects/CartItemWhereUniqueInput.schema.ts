...
import { CartItemCart_idProduct_idCompoundUniqueInputObjectSchema } from './CartItemCart_idProduct_idCompoundUniqueInput.schema'

const makeSchema = () => z.object({
  id: z.string().optional(),
  cart_id_product_id: z.lazy(() => CartItemCart_idProduct_idCompoundUniqueInputObjectSchema).optional()
}).strict().superRefine((obj, ctx) => {
        const present = (k: string) => (obj as any)[k] != null;
        const singles: string[] = ["id"] as string[];
        const groups: string[][] = [["cart_id","product_id"]] as string[][];
